...
from leap.models.ME_U0.lance_src.common.utils.distributed import get_device
...
class WanVideoVAE(object):
    ...
    def __init__(self, config_path: str = "", **kwargs) -> None:
        ...
        # lance_vla: allow an explicit checkpoint path (the vendored Wan2.2_VAE.pth)
        # to bypass config/path_default.yaml resolution.
        self._vae_pth = kwargs.get("vae_pth", "")
    ...
    def configure_vae_model(self):
        ...
        # lance_vla: prefer an explicitly provided checkpoint path.
        if getattr(self, "_vae_pth", ""):
            vae_path = self._vae_pth
        else:
            # Read the VAE path from path_default.yaml.
            try:
                from leap.models.ME_U0.lance_src.config.config_factory import get_model_path
