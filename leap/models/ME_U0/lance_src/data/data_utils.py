...
def create_sparse_mask(document_lens, split_lens, attn_modes, device, token_valid=None):
    ...
    def padding_mask(b, h, q_idx, kv_idx):
        query_valid = token_valid[q_idx]
        key_valid = token_valid[kv_idx]
        return (query_valid & key_valid) | (~query_valid & (q_idx == kv_idx))
    ...
    mask = and_masks(or_masks(causal_mask, full_and_noise_mask), remove_noise_mask, sample_mask)
    if token_valid is None:
        return mask

    token_valid = torch.as_tensor(token_valid, dtype=torch.bool, device=device)
    expected_len = sum(document_lens)
    if token_valid.numel() != expected_len:
        raise ValueError(
            f"token_valid has {token_valid.numel()} entries, expected {expected_len}"
        )
    return and_masks(mask, padding_mask)
