...
import subprocess
from .._utils import get_logger, dict_apply
...
def load_lerobot_v2_minmax(dataset_dirs, shape_meta):
    """Project native v2.1 episode min/max into ME-U0's field statistics.

    Read metadata only; do not sample action windows or write a stats cache.
    These statistics apply before any action/state transform, so callers must
    use native fields (optionally selected by index) and global normalization.
    """
    native = {}
    fields = [m for role in ("action", "state") for m in shape_meta[role]]
    features = {m["lerobot_key"] for m in fields}
    for directory in dataset_dirs:
        meta_dir = Path(directory) / "meta"
        info = json.loads((meta_dir / "info.json").read_text())
        if info["codebase_version"] != "v2.1":
            raise ValueError(f"native min/max requires LeRobot v2.1: {directory}")
        episodes = {
            json.loads(line)["episode_index"]
            for line in (meta_dir / "episodes.jsonl").read_text().splitlines() if line
        }
        seen = set()
        with (meta_dir / "episodes_stats.jsonl").open() as handle:
            for line in handle:
                entry = json.loads(line)
                episode = entry["episode_index"]
                if episode in seen:
                    raise ValueError(f"duplicate episode stats: {directory}, {episode}")
                seen.add(episode)
                for feature in features:
                    bounds = {
                        name: torch.as_tensor(entry["stats"][feature][name], dtype=torch.float32).flatten()
                        for name in ("min", "max")
                    }
                    expected = tuple(info["features"][feature]["shape"])
                    if any(tuple(x.shape) != expected or not torch.isfinite(x).all() for x in bounds.values()):
                        raise ValueError(f"invalid min/max for {feature}: {directory}, {episode}")
                    if torch.any(bounds["min"] > bounds["max"]):
                        raise ValueError(f"inverted min/max for {feature}: {directory}, {episode}")
                    if feature not in native:
                        native[feature] = bounds
                    else:
                        native[feature]["min"] = torch.minimum(native[feature]["min"], bounds["min"])
                        native[feature]["max"] = torch.maximum(native[feature]["max"], bounds["max"])
        if seen != episodes:
            raise ValueError(f"episode stats do not cover episodes.jsonl: {directory}")
    result = {"action": {}, "state": {}}
    for role in result:
        for field in shape_meta[role]:
            if field.get("canonical_names") or field["shape"] != field["raw_shape"]:
                raise ValueError("native min/max requires untransformed, index-selected fields")
            stats = native[field["lerobot_key"]]
            selected = {
                "global_" + name: value[field["select_indices"]] if "select_indices" in field else value.clone()
                for name, value in stats.items()
            }
            if any(x.numel() != int(field["raw_shape"]) for x in selected.values()):
                raise ValueError(f"native stats width does not match {field['key']}")
            result[role][field["key"]] = selected
    return result
...
def dataset_stats_payload_to_tensors(
    payload: Dict[str, Any], try_convert_tensor: bool = True
) -> Dict[str, Any]:
...
def load_dataset_stats_from_json(file_path: str,
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    return dataset_stats_payload_to_tensors(
        data, try_convert_tensor=try_convert_tensor
    )
