...
from .._utils import dict_apply, get_logger
from .base_processor import BaseProcessor

logger = get_logger(__name__)

class MEU0Processor(BaseProcessor):
    def __init__(
        self,
        # keys
        ...
        self.native_shape_meta = shape_meta
        ...
        if delta_action_dim_mask is None:
            ...
            action_meta = self.native_shape_meta["action"]
    ...
    @property
    def requires_normalizer_stats(self) -> bool:
        """Whether the owning dataset must load/build legacy native stats."""
        return True
    ...
    def augment_instruction(self, data: Dict[str, str] | List[str]) -> List[str]:
        ...
        # The formal Lance route uses drop_high_level_prob=1.0.  Avoid touching
        # the process-global NumPy RNG in that deterministic case: DataLoader
        # prefetch depth or an exact-resume restart must not advance unrelated
        # augmentation state merely to select a branch whose outcome is fixed.
        if self.drop_high_level_prob >= 1.0:
            instruction = f"{low_level_instruction}"
        elif self.drop_high_level_prob <= 0.0:
            instruction = f"[High]: {high_level_instruction}, [Low]: {low_level_instruction}"
        elif np.random.rand() < self.drop_high_level_prob:
            instruction = f"{low_level_instruction}"
        else:
            instruction = f"[High]: {high_level_instruction}, [Low]: {low_level_instruction}"
    ...
    def action_state_transform(self, batch):
        if "action" in batch:
            for meta in self.native_shape_meta["action"]:
        ...
        for meta in self.native_shape_meta["state"]:
            k, meta_shape = meta["key"], meta["raw_shape"]
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for meta in self.shape_meta["images"]:
            ...
            meta_shape = [self.num_obs_steps] + list(shape)
            assert list(image.shape) == meta_shape, \
                f"Expected shape {meta_shape}, got {image.shape} after transforms for key {key}"

            processed_images.append(image)
        pixel_values = torch.stack(processed_images, dim=0) # [num_cameras, T, C, H, W]
        # Each shard is single-source with a fixed camera count, so no padding or
        # truncation is done: num_output_cameras must equal the actual number of
        # cameras. camera_is_pad is not emitted (the model treats its absence as
        # "no padded cameras").
        assert pixel_values.shape[0] == self.num_output_cameras, (
            f"num_output_cameras ({self.num_output_cameras}) must equal the actual "
            f"number of cameras ({pixel_values.shape[0]})."
        )
        ...
        # Keep episode-local absent dimensions neutral after normalization.  A
        # padded zero can otherwise become non-zero after min/max or z-score
        # normalization and leak a fake hand signal into the model condition.
        if "action" in data:
            data["action"][..., data["action_dim_is_pad"]] = 0.0
        data["state"][..., data["state_dim_is_pad"]] = 0.0
